...
const wall_jump_pushback = 1200
const wall_slide_gravity = 3
#dash
...
func _physics_process(delta: float) -> void:
	if is_on_wall_only() and Input.is_action_pressed("left") or is_on_floor_only() and Input.is_action_pressed("right"):
		velocity.y = delta * wall_slide_gravity
	if !is_on_floor():
		velocity.y += GRAVITY
		if velocity.y > 1000:
			velocity.y = 1000
